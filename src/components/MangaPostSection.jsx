...
// Popular List Component for Home Page
function PopularList({ mangaList }) {
  const { toggleBookmark, isBookmarked, canAddMore } = useBookmarks();
  ...
  const handleBookmarkClick = (e, id) => {
    ...
    if (isBookmarked(id)) {
      toggleBookmark(id); // remove
    } else if (canAddMore()) {
      toggleBookmark(id); // add
    }
  };
  ...
  return (
    <div className="space-y-4">
      {mangaList.map((manga, index) => {
        ...
        // ✅ add cache-busting param to avoid stale covers
        const imageUrl = fileName
          ? `https://uploads.mangadex.org/covers/${id}/${fileName}.256.jpg?t=${Date.now()}`
          : "/default-cover.png";
...
export default function MangaPostSection({ section }) {
    ...
    const fetchManga = async () => {
      try {
        ...
        if (section === "popular") {
          ...
          mangaData = res.data.data.slice(0, 10);
        }
        ...
        if (error.response?.status === 403) {
          toast.error(
            "⚠️ Access blocked: CORS or API issue. Try refreshing later."
          );
        } else if (error.response?.status === 429) {
          toast.error("🚫 Too many requests. Please slow down.");
        } else {
  ...
  // 🔄 ... keep your getSectionConfig and render logic the same
  // (not repeating here for brevity, only changed PopularList + cover urls)
  //   // Get section configuration
  const getSectionConfig = (section) => {
    switch (section) {
      case "what's new":
