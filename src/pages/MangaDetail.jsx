...
import { Heart, Star, Share2, Play, Clock, Users, Tag, BookOpen } from "lucide-react";
import { fetchChapters, fetchMangaDetail, fetchRelatedByAuthor, getCoverUrl, getMangaTitle } from "../services/mangaService";
      ...
      try {
        ...
        if (firstBatch.length === 0) {
          ...
          const allEn = firstBatch.every(
            (c) => (c.attributes?.translatedLanguage || "").toLowerCase() === "en"
          );
        ...
        if (authorRel?.id) {
          const rel = await fetchRelatedByAuthor({ authorId: authorRel.id, excludeId: id, limit: 6 });
               ...
               <div className="sticky top-8">
                 ...
                 <div className="mt-6 space-y-3">
                   ...
                   >
                     ...
                     <span>{chapters.length > 0 ? "Start Reading" : "No Chapters Available"}</span>
                   </button>
                    ...
                    >
                      <Heart
                        className={`w-5 h-5 ${isBookmarked(id) ? "fill-current" : ""}`}
                      />
                      <span>{isBookmarked(id) ? "Saved" : "Save to Library"}</span>
                    </button>
                 </div>
               </div>
             </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-sm">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span>
                        Updated {new Date(manga.attributes?.updatedAt || Date.now()).toLocaleDateString()}
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(manga.attributes?.tags || []).map((tag) => {
                      const name = tag?.attributes?.name?.en ||
                        (tag?.attributes?.name && Object.values(tag.attributes.name)[0]) ||
                        "Tag";
                      return (
                        <span
                          key={tag.id}
                  ...
                  <div className="bg-[#1a1a2e] rounded-xl p-4 border border-[#2a2a4e]">
                    <h4 className="text-[#c77dff] font-semibold mb-1">
                     ...
                     <p className="text-white">{
                       manga.relationships?.find((r) => r.type === "artist")?.attributes?.name ||
                       "Unknown"
                     }</p>
                  </div>
                </div>
          ...
          <div className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="bg-[#2a2a4e] rounded-xl p-4">
                 ...
                 <div className="bg-[#2a2a4e] rounded-xl p-4">
                  <h4 className="text-[#c77dff] font-semibold mb-2">Language</h4>
                ...
                <div className="bg-[#2a2a4e] rounded-xl p-4">
                  ...
                  <p className="text-white">{manga.attributes?.contentRating || "Safe"}</p>
                </div>
              </div>
            )}
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                   <div className="flex items-center gap-3">
                     <h3 className="text-xl font-semibold text-white">Latest Chapters</h3>
                     <span className="text-xs px-2 py-1 rounded-full border border-[#2a2a4e] text-gray-300">Lang: {chapterLangLabel}</span>
                   </div>
                ...
                <div className="space-y-3">
                  {chapters.length === 0 && (
                    <div className="bg-[#2a2a4e] rounded-xl p-6 text-center">
                      <div className="text-gray-400 mb-4">
                        ...
                        <p className="text-lg font-semibold">No chapters available</p>
                        <p className="text-sm">This manga may not have any chapters uploaded yet or they may not be available in your preferred language.</p>
                      </div>
                      <button
                  ...
                  {chapters.length > 0 && chapterLangLabel !== "EN" && (
                    <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="text-yellow-400 font-semibold">Language Notice</p>
                          <p className="text-yellow-300 text-sm">Some chapters may not be available in English. You can still read them, but the content will be in the original language.</p>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {chapters.map((chapter) => {
                    const chapterLanguage = chapter.attributes?.translatedLanguage?.toLowerCase();
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between">
                            ...
                            <div>
                              ...
                              <div className="flex items-center space-x-2 mt-1">
                                <p className="text-gray-400 text-sm">
                                  {new Date(chapter.attributes?.readableAt || chapter.attributes?.createdAt || Date.now()).toLocaleDateString()}
                                </p>
                          ...
                          <div className="text-gray-400 text-sm">ID: {chapter.id.slice(0, 6)}…</div>
                        </div>
                      </div>
                    );
                  })}
        ...
        <div className="bg-[#1a1a2e] rounded-2xl p-6 border border-[#2a2a4e]">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {related.map((m) => {
              ...
              return (
                <div key={m.id} onClick={() => navigate(`/manga/${m.id}`)} className="bg-[#2a2a4e] rounded-2xl p-2 hover:bg-[#3a3a5e] transition-colors duration-300 cursor-pointer group">
                  <div className="aspect-[2/3] rounded-lg mb-2 overflow-hidden">
                    <img src={img} alt={title} className="w-full h-full object-cover" />
                  </div>
                  <h4 className="text-white text-sm font-semibold truncate group-hover:text-[#c77dff] transition-colors duration-300">{title}</h4>
                </div>
              );
            })}
            {related.length === 0 && (
